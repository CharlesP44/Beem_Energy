...
def _build_api_urls(battery_id: int | None) -> dict:
    ...
    if battery_id:
        urls["battery_charged"] = (
            f"https://api-x.beem.energy/beemapp/batteries/{battery_id}/energy-charged/intraday"
        )
        urls["battery_discharged"] = (
            f"https://api-x.beem.energy/beemapp/batteries/{battery_id}/energy-discharged/intraday"
        )
    return urls
...
async def async_export_to_csv(hass: HomeAssistant, service_call: ServiceCall):
    ...
    for device_id in device_ids:
        ...
        _LOGGER.info(
            "Exportation CSV pour le compte %s (appareil %s), de %s à %s",
            entry_id,
            device_id,
            start_date,
            end_date,
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for api_name, api_url in API_URLS.items():
                _LOGGER.info(
                    f"📡 Traitement de : {api_name} pour l'appareil {device_id}"
                )
                ...
                while cur_from < end_dt:
                    ...
                    try:
                        ...
                        devices = (
                            data.get("devices")
                            or data.get("houses")
                            or ([data] if "batteryId" in data else [])
                        )
                        for device_data in devices:
                            dev_id = (
                                device_data.get("deviceId")
                                or device_data.get("houseId")
                                or device_data.get("batteryId", "N/A")
                            )
                            for measure in device_data.get("measures", []):
                                start_iso, val = (
                                    measure.get("startDate"),
                                    measure.get("value", 0),
                                )
                                if start_iso and val is not None:
                                    dt_utc = dt_util.parse_datetime(start_iso)
                                    dt_paris = dt_utc.astimezone(
                                        ZoneInfo("Europe/Paris")
                                    )
                                    all_rows.append(
                                        {
                                            "startDate_utc": dt_utc.isoformat(),
                                            "datetime_paris": dt_paris.strftime(
                                                "%Y-%m-%d %H:%M:%S"
                                            ),
                ...
                if all_rows:
                    ...
                    await hass.async_add_executor_job(
                        _write_csv_sync,
                        file_path,
                        sorted(all_rows, key=lambda x: x["datetime_paris"]),
                        fieldnames,
                    )
    ...
    await hass.services.async_call(
        ...
        "create",
        {"title": "Beem Energy Export", "message": "Exportation CSV terminée."},
    )
...
async def async_export_for_import(hass: HomeAssistant, service_call: ServiceCall):
    ...
    for device_id in device_ids:
        ...
        entry_id = list(device.config_entries)[0]
        _LOGGER.info(
            "Export (format import) pour le compte %s de %s à %s",
            entry_id,
            start_date,
            end_date,
        )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for api_name, api_url in API_URLS.items():
                _LOGGER.info(
                    f"Traitement (format import) de : {api_name} pour l'appareil {device_id}"
                )
                        ...
                        for device_data in devices:
                            for measure in device_data.get("measures", []):
                                ...
                                if start_iso and val is not None:
                                    raw_measures.append(
                                        {
                                            "start": dt_util.parse_datetime(start_iso),
                                            "value": float(val),
                                        }
                                    )
                    except Exception as e:
                ...
                if api_name == "production":
                    hourly_agg = {
                        m["start"].strftime("%Y-%m-%d %H:00:00"): 0.0
                        for m in raw_measures
                    }
                    for m in raw_measures:
                        hourly_agg[m["start"].strftime("%Y-%m-%d %H:00:00")] += m[
                            "value"
                        ]
                    processed_measures = [
                        {
                            "start": datetime.fromisoformat(k).replace(
                                tzinfo=timezone.utc
                            ),
                            "value": v,
                        }
                ...
                for measure in processed_measures:
                    ...
                    all_rows_for_import.append(
                        {
                            "statistic_id": statistic_id,
                            "unit": "kWh",
                            "start": dt_paris.strftime("%d.%m.%Y %H:%M"),
                            "state": "",
                            "sum": round(cumulative_sum_kwh, 6),
                        }
                    )
                ...
                _LOGGER.info(
                    f"✅ Fichier (format import) exporté : /local/beem_exports/{filename}"
                )
    ...
    await hass.services.async_call(
        ...
        "create",
        {
            "title": "Beem Energy Export",
            "message": "Exportation (format import) terminée.",
        },
    )
...
async def async_export_for_ha_import(hass: HomeAssistant, service_call: ServiceCall):
    ...
    for device_id in device_ids:
        ...
        entry_id = list(device.config_entries)[0]
        _LOGGER.info(
            "Export (format HA) pour le compte %s de %s à %s",
            entry_id,
            start_date,
            end_date,
        )
        ...
        if not main_serial_lower:
            _LOGGER.error(
                "Impossible de trouver le numéro de série principal pour le compte %s.",
                entry_id,
            )
            continue
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for api_name, (source_key, mode) in API_TO_SENSOR_MAP.items():
                if api_name.startswith("battery_") and not battery_id:
                    continue
                ...
                if not entity_id:
                    _LOGGER.warning(
                        f"Capteur avec unique_id '{unique_id}' non trouvé, export ignoré."
                    )
                    continue

                statistic_id = entity_id
                _LOGGER.info(
                    f"Traitement (format HA) de : {api_name} -> {statistic_id}"
                )
                ...
                _LOGGER.info(
                    f"✅ Fichier (format HA) exporté : /local/beem_exports/{filename}"
                )

    await hass.services.async_call(
        "persistent_notification",
        "create",
        {"title": "Beem Energy Export", "message": "Exportation (format HA) terminée."},
    )
...
def async_unload_export_services(hass: HomeAssistant):
