...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if task_bat and not task_bat.done():
        _LOGGER.info(
            "Annulation de la tâche MQTT Cloud Beem pour l'entrée: %s", entry.entry_id
        )
        task_bat.cancel()
        try:
            ...
            _LOGGER.warning(
                "La tâche MQTT Cloud Beem n'a pas répondu à l'annulation dans les temps."
            )
        except Exception as e:
            _LOGGER.error(
                "Erreur inattendue en attendant l'arrêt de la tâche MQTT: %s", e
            )
