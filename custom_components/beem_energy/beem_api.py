...
async def get_houses(token_rest: str, page_offset: int = 1, page_size: int = 50) -> list:
    """Récupère la liste des maisons (paginated endpoint).

    Retourne la liste d'objets maison (la clé 'data' du payload paginé).
    """
    url = f"{BASE_URL}/house/paginated"

    # Respect global 429 lock
    if _global_429_is_locked():
        raise BeemConnectionError("Global API rate limit in effect — try later")

    payload = {"pageOffset": int(page_offset), "pageSize": int(page_size), "searchText": ""}
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, headers=headers, json=payload) as resp:
                ...
                if resp.status == 429:
                    _global_429_set_lock()
                    raise BeemConnectionError("Limite API atteinte (429) sur /house/paginated")
                if resp.status not in (200, 201):
                    _LOGGER.warning("Erreur récupération house/paginated (%s) : %s", resp.status, text)
                    return []
                _global_429_reset()
                data = await resp.json()
                return data.get("data", [])
    except aiohttp.ClientError as e:
        _LOGGER.error("Erreur réseau Beem (house/paginated): %s", e)
        return []
    except Exception as e:
        _LOGGER.error("Erreur inattendue Beem (house/paginated): %s", e)
        return []
    now = datetime.now()
